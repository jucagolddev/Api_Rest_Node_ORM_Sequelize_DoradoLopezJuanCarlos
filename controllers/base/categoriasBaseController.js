/**
 * Controlador Base para Categorias.
...
export const obtenerTodos = async (req, res) => {
  try {
    const lista = await CategoriasService.findAll();
    res.json(lista);
  } catch (error) {
...
export const obtenerUno = async (req, res) => {
  try {
    const item = await CategoriasService.findById(req.params.id);
...
export const actualizar = async (req, res) => {
  try {
    const actualizado = await CategoriasService.update(req.params.id, req.body);
...
export const eliminar = async (req, res) => {
  try {
    const eliminado = await CategoriasService.remove(req.params.id);
